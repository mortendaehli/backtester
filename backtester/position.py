...
class Position(object):
    # Initializing values as none
    market_value: Optional[float]
    avg_price: Optional[float]
    net: Optional[float]
    net_total: Optional[float]
    net_incl_comm: Optional[float]
    cost_basis: Optional[float]

    def __init__(self, action: str, ticker: str, init_quantity: float, init_price: float, init_commission: float, bid: float, ask: float) -> None:
        """
    ...
    def _calculate_initial_value(self) -> None:
        """
    ...
    def update_market_value(self, bid: float, ask: float) -> None:
        """
    ...
    def transact_shares(self, action: str, quantity: float, price: float, commission: float) -> None:
        """
        ...
        if action == "BOT":
            ...
            if self.action != "SLD":  # Increasing long position
                ...
                self.realised_pnl += quantity * (self.avg_price - price) - commission  # Adjust realised PNL
            self.buys += quantity
            self.total_bot = self.buys * self.avg_bot
